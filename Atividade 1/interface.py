...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.btnVizualizar.clicked.connect(self.btnVizualizarClick)
        ...
        self.btnCarregarDadosUrl.clicked.connect(self.carregarDadosUrlClick)
    ...
    def btnVizualizarClick(self):
        try:
            ...
            arq = open('HTML/tables.html', 'r')
            arquivoHtml = str(arq.read())
            arq.close()

            trBase = "<tr><td>element</td></tr>"

            arquivoHtml = arquivoHtml.replace("trLabelReplaceFinalVerbos",self.gerarTrs(trBase,a.verbos))
            arquivoHtml = arquivoHtml.replace("trLabelReplaceVerboAuxiliares", self.gerarTrs(trBase,a.verbos_aux))
            arquivoHtml = arquivoHtml.replace("trLabelReplaceFinalPronomes", self.gerarTrs(trBase,a.pronomes))
            arquivoHtml = arquivoHtml.replace("trLabelReplaceFinalSubstantivosComuns", self.gerarTrs(trBase,a.subst_comuns))
            arquivoHtml = arquivoHtml.replace("trLabelReplaceFinalSubstantivosProprios", self.gerarTrs(trBase,a.subst_proprios))
            arquivoHtml = arquivoHtml.replace("trLabelReplaceFinalConjuncoes", self.gerarTrs(trBase,a.conjuncoes))

            arquivoHtml = arquivoHtml.replace("conjuncoesName", "Conjunções")
            arquivoHtml = arquivoHtml.replace("subPName", "Substantivos Próprios")
            arquivoHtml = arquivoHtml.replace("namePage", "Resultado das Análises")

            arq = open('HTML/tables1.html', 'w',encoding="utf-8")
            arq.write(arquivoHtml)
            arq.close()
    ...
    def carregarDadosUrlClick(self):
        try:
            self.plainTextEdit_2.setPlainText(Web_Scraping.processar(self.inputUrl.text()))
        except Exception as e:
            self.alert(title="Erro", information="Não foi possível esta operação, por favor tente uma nova url.",
                       detail=str(e))

    def processarTr(self,trBase,element):
        return str(trBase).replace("element", element)

    def gerarTrs(self, trBase, array):
        trsFinal = ""
        for i in array:
            trsFinal += self.processarTr(trBase, i)
